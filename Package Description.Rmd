...
```{r, echo=FALSE}
print("data<- data %>% transmute(A1 = `A1 F1 I1 R1` + `A1 F1 I2 R1`, A2 = `A2 F1 I1 R1`+ `A2 F1 I2 R1`, A3 = `A3 F1 I1 R1`+ `A3 F1 I2 R1`, A4 = `A4 F1 I1 R1`+ `A4 F1 I2 R1`, A5 = `A5 F1 I1 R1`+ `A5 F1 I2 R1`, A6 = `A6 F1 I1 R1`+ `A6 F1 I2 R1`, A7 = `A7 F1 I1 R1`+ `A7 F1 I2 R1`, A8 = `A8 F1 I1 R1`+ `A8 F1 I2 R1`, A9 = `A9 F1 I1 R1`+ `A9 F1 I2 R1`, A10 = `A10 F1 I1 R1`+ `A10 F1 I2 R1`,A11 = `A1 F1 I1 R1`+ `A11 F1 I2 R1`, A12 = `A12 F1 I1 R1`+ `A12 F1 I2 R1`, A13 = `A13 F1 I1 R1`+ `A13 F1 I2 R1`, A14 = `A14 F1 I1 R1`+ `A14 F1 I2 R1`) %>% replace(is.na(.), 0)")  
```
I used the **transmute** function to combine both columns, then name them by their population name, A1, A2, and so on as well as delete the old columns. I replaced the *NA* with 0 with replace(is.na(.), 0)) and now the dataset looks like this:  
```{r, echo = FALSE, results = 'hide', warning=FALSE , message=FALSE}
library(tidyverse)
data <- read_csv("~/Desktop/Co-op/other/evolve-resequence-constraint/data-out/Wannier2018example_Analysis.csv")
```
```{r }
head(data, n=6L)
```
...
```{r, echo = FALSE, results = 'hide', warning=FALSE , message=FALSE}
library(tidyverse)
data <- read_csv("~/Desktop/Co-op/other/evolve-resequence-constraint/data-out/Wannier2018example2_Analysis.csv")
```
```{r }
head(data, n=6L)
```
...
```{r, echo = FALSE, results = 'hide', warning=FALSE , message=FALSE}
library(tidyverse)
data <- read_csv("~/Desktop/Co-op/other/evolve-resequence-constraint/data-out/Tenaillon2016_example_Analysi.csv")
```
```{r }
head(data, n=6L)
```
...
```{r, echo = FALSE, results = 'hide', warning=FALSE , message=FALSE}
library(tidyverse)
data <- read_csv("~/Desktop/Co-op/other/evolve-resequence-constraint/data-in/Tenaillon2016.csv")
```
```{r }
head(data, n=6L)
```
...
```{r, echo=FALSE}
print("data <- data %>% 
  transmute(`500` = `500 I1 R1`+`500 I2 R1`, `1000` = `1000 I1 R1`+`1000 I2 R1`, `1500` =`1500 I1 R1`+`1500 I2 R1`, `2000` = `2000 I1 R1`+`2000 I2 R1`, `5000` = `5000 I1 R1`+`5000 I2 R1`, `10000` = `10000 I1 R1`+`10000 I2 R1`, `15000` = `15000 I1 R1`+`15000 I2 R1`, `20000` = `20000 I1 R1`+`20000 I2 R1`, `30000` = `30000 I1 R1`+`30000 I2 R1`, `40000` = `40000 I1 R1`+`40000 I2 R1`, `50000` = `50000 I1 R1`+`50000 I2 R1`) %>% replace(is.na(.), 0) ")
```
...
```{r, echo = FALSE, results = 'hide', warning=FALSE , message=FALSE}
library(tidyverse)
data <- read_csv("data-out/Tenaillon2016_example_Analysis.csv")
```
```{r }
head(data, n=6L)
```
